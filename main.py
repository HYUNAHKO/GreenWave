...
# 5. 최종 시각화: 4개의 서브플롯 생성 및 저장 기능 추가
def visualize_results(original_image, segmented_mask, b_image_path, final_image, save_path=None):
    """
    최종 결과를 시각화합니다. 서브플롯은 총 4개입니다.
    """
    ...
    # 2. 세그멘테이션 결과 이미지 (바다 vs 비 바다)
    plt.subplot(1, 4, 2)
    plt.imshow(segmented_mask, cmap='gray')
    plt.title("Segmentation Result (Ocean vs Non-Ocean)")

    # 3. 히트맵이 적용된 이미지 'B.png'
    plt.subplot(1, 4, 3)
    plt.imshow(np.array(Image.open(b_image_path).convert('RGB')))  # alpha 채널 제거 후 RGB 이미지 사용
    plt.title("Log-Adjusted Heatmap Image (B)")
    ...
    # 결과 저장 (저장 경로가 주어진 경우)
    if save_path:
        plt.savefig(os.path.join(save_path, 'final_result.png'), dpi=300, bbox_inches='tight')
...
def main(image_path, model_path, save_path):
    ...
    # 'B.png' 경로 설정
    b_image_path = os.path.join(save_path, 'B.png')

    # 최종 이미지 'D' 생성 (이미지 'B'에 비 바다 영역의 원본 RGB를 덮어씌움)
    final_image = create_final_image(b_image_path, segmented_mask, non_ocean_rgb)

    # 결과 시각화 및 저장
    visualize_results(original_image, segmented_mask, b_image_path, final_image, save_path)
